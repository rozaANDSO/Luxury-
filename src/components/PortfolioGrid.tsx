...
export default function PortfolioGrid({ items }: PortfolioGridProps) {
  ...
  const filteredItems: Project[] = activeCategory === 'all'
    ? safeItems.filter((item) => item.title !== 'Coming Soon') // Exclude placeholders for 'All'
        ...
        {filteredItems.map((project) => (
          ...
          >
            <div className="relative aspect-square">
              {project.images?.[0] ? (
                <Image
                  src={project.images[0].path}       // <--- Use .path string here
                  alt={project.images[0].title || project.title}
                  fill
